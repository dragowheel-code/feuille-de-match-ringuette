...
export default function PunitionModal({
  ...
  const selections = [joueusePunition, joueusePurgeePar].filter(Boolean)

  function cliquerJoueuse(joueuse) {
    const numero = String(joueuse.numero)

    const nouvellesSelections = [...selections]

    const indexExistant = nouvellesSelections.findIndex(
      (selection) => String(selection) === numero
    )

    if (indexExistant >= 0) {
      nouvellesSelections.splice(indexExistant, 1)
    } else if (nouvellesSelections.length < 2) {
      nouvellesSelections.push(numero)
    }

    setJoueusePunition(nouvellesSelections[0] ?? '')

    setJoueusePurgeePar(nouvellesSelections[1] ?? '')
  }

  function obtenirJoueuseParNumero(numero) {
    if (!numero) {
      return null
    }
    ...
    return (
      joueusesPunitionDisponibles.find(
        (joueuse) => String(joueuse.numero) === String(numero)
      ) ?? null
    )
  }

  const joueusePunitionSelectionnee = obtenirJoueuseParNumero(joueusePunition)

  const joueusePurgeeParSelectionnee = obtenirJoueuseParNumero(joueusePurgeePar)

  const joueusesTriees = [...joueusesPunitionDisponibles].sort(
    (a, b) => Number(a.numero) - Number(b.numero)
  )

  function changerEquipe(nouvelleEquipe) {
    setEquipePunition(nouvelleEquipe)
    ...
    <div className="modal-backdrop">
      <div className="modal">
        ...
        <select
          ...
          onChange={(e) => changerEquipe(e.target.value)}
        >
        ...
        <input
          ...
          onBlur={() =>
            setTempsPunitionTableau(
              formaterTempsPendantSaisie(tempsPunitionTableau)
            )
          }
        ...
        <p>
          Cliquez d'abord le numéro de la joueuse punie, puis celui de la
          joueuse qui purge si nécessaire.
        </p>

        <div className="grille-numeros-joueuses">
          {joueusesTriees.map((joueuse) => {
            const numero = String(joueuse.numero)

            const indexSelection = selections.findIndex(
              (selection) => String(selection) === numero
            )

            const selectionnee = indexSelection >= 0
            ...
            return (
              <button
                ...
                type="button"
                className={
                  selectionnee ? 'numero-joueuse selectionne' : 'numero-joueuse'
                }
                onClick={() => cliquerJoueuse(joueuse)}
              >
                <strong>#{joueuse.numero}</strong>

                {selectionnee && (
                  <small>{indexSelection === 0 ? 'P' : 'PP'}</small>
                )}
              </button>
            )
          })}
