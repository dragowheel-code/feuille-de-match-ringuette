...
function GrilleChandails({
  ...
  const taillesTriees = trierTailles(Object.keys(ensemblesParTaille))

  const [taillesFermees, setTaillesFermees] = useState(
    () => new Set(taillesTriees)
  )

  function basculerTaille(taille) {
    setTaillesFermees((precedentes) => {
      const nouvelles = new Set(precedentes)

      if (nouvelles.has(taille)) {
  ...
  function tailleEstOuverte(taille) {
    return !taillesFermees.has(taille)
  }

  return (
    <section className="grille-chandails">
      {taillesTriees.map((taille) => {
        const ensembles = ensemblesParTaille[taille]

        const ouverte = tailleEstOuverte(taille)

        return (
          <section key={taille} className="groupe-taille">
            <button
              type="button"
                ...
                {ensembles.map((ensemble) => {
                    ...
                    attributionsChandails.find(
                      (attribution) =>
                        String(attribution.ensembleId) ===
                          String(ensemble.id) && attribution.active === true
                    ) ?? null
                  ...
                  return (
                    <CarteEnsembleChandails
                      ...
                      selectionne={
                        String(ensembleSelectionne?.id) === String(ensemble.id)
                      }
                      selectionner={selectionnerEnsemble}
                    />
                  )
                })}
              </div>
